...
В Rust система типов была спроектирована так, что бы больше быть похожей на 
...
Другими словами `Vec<isize>` и `Vec<char>` *это два разных типа*, которые 
...
Это особое поведение для создания абстрактных параметров требует специального 
`impl` блока. И приводит к тому, что меняя значения в реализации
абстрактного типа мы создает разные конечные типы основанные на разных реализациях
`impl` блоков.

В тоже время в объектно ориентированных языках классы могут наследовать поведение 
своих родителей. Это позволяет расширять типы не только с помощью новых черт в
дочерних классах, но и создавать/расширять их каждым новым.
...
Зазпросы от лабораторных машин будут содержать одну и ту же информацию с
...
до момента времени выполнения нашего приложения. Это означает, что 
любой метод который будет применяться только к одному из протоколов имел
проверки что он не будет применен к другому во время выполнения.

Вот так будет выгляеть получение точки монтирования для NFS:
...
Все это приводит к тому, что при каждом вызыве `mount_point()` теперь надо провероять
не возвращает ли этот метод `None` и писать код который будет обрабатывать это.
И это все верно, даже если мы можем быть уверены что в текущей ситуации мы будет
запрашивать только NFS.
